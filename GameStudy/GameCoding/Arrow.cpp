...
{
    Super::BeginPlay();
    SetState(ObjectState::Move); // 초기 상태를 이동으로 설정
    UpdateAnimation();
}
...
void Arrow::TickMove()
{
    ...
    Vec2 moveDir = (_destPos - _pos);

    if (moveDir.Length() < 5.0f)
    {
        // 목표 위치에 도달
