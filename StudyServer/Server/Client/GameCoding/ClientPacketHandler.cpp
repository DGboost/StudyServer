	...
	if (scene)
	{
		...
		if (attacker)
		{
			...
			{				GameObject* target = scene->GetObject(targetId);				if (target)				{
					...
					cout << "[HitEffect Debug]   - LogicalPos: (" << targetLogicalPos.x << ", " << targetLogicalPos.y << ")" << endl;
					cout << "[HitEffect Debug]   - VisualPos: (" << targetVisualPos.x << ", " << targetVisualPos.y << ")" << endl;
					*/// 히트 이펙트 생성 - 플레이어와 동일한 렌더링 방식 적용
					Vec2 targetWorldPos;
